...
import { Signal } from "@/lib/altuq/types";
...
export function SignalFeed({ signals, isLoading }: SignalFeedProps) {
    if (isLoading) return <div className="text-zinc-500">Sinyaller yükleniyor...</div>;
    if (!signals?.length) return <div className="text-zinc-500">Aktif sinyal yok.</div>;
                    ...
                    <div className="p-6 space-y-4">
                        ...
                        <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                                ...
                                <div className="text-[10px] text-zinc-500 uppercase font-medium">Güç</div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2 pt-2">
                            <p className="text-xs font-medium text-zinc-400">Tespit Edilen Göstergeler:</p>
                        ...
                        <div className="pt-4 flex items-center justify-between gap-4 border-t border-white/5 mt-4">
                            <div className="space-y-0.5">
                                <div className="text-[10px] text-zinc-500 uppercase">Önerilen Giriş</div>
                                <div className="text-sm font-mono text-white">
                                    {signal.suggested_entry[0].toLocaleString()} - {signal.suggested_entry[1].toLocaleString()}
                                </div>
                            </div>

                            <Button size="sm" className="bg-white/10 hover:bg-white/20 text-white gap-2 h-9 transition-colors">
                                Detay
